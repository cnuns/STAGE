...
class CoverageWrapper(Coverage):
    ...
    def apply_fault(self, obs_n):
        ...
        F_comp = 1.0 - F
        obs_n[:, :agent_filter] += obs_n[:, agent_filter:agent_filter*2] * F_comp #fault된 agent를 obstacle로 반영

        F_obst = (obs_n[:, agent_filter:agent_filter*2] * F_comp).reshape((Na,) + self._agent_view_mask) # 실제 폴트 발생해서 에이전트가 obst로 반영된 좌표
        fault_neighbor = binary_dilation(F_obst, structure=STRUCT_2D[None, :, :]) # 상하좌우로 1 퍼뜨리기
        fault_neighbor = fault_neighbor.reshape(Na, agent_filter)

        is_visited = self.visited_to_local_view().reshape(Na, agent_filter) # 에이전트 좌표를 중앙으로 해서 visited 맵 가져 옴

        obs_n[:, agent_filter*2:agent_filter*3] = np.where(~is_visited.astype(bool) & ~obs_n[:, :agent_filter].astype(bool) & fault_neighbor, 0, obs_n[:, agent_filter*2:agent_filter*3]) # grid cell에 fault 적용 -> visit하지 않고 장애물이 없고 fault 발생 주변이면 0으로 만듦(fault때문에 incentive cell인 것처럼 보임)

        obs_n[:, agent_filter:agent_filter*2] *= F #agent에 fault 적용

        return [(obs_n).flatten()] if is_list else (obs_n).flatten()

    def visited_to_local_view(self):

        patch_size = self._agent_view_mask[0]
        pad_width = patch_size // 2  # 5x5일 경우 사방에 2칸씩 패딩 필요
        
        # 1. 범위 밖을 1로 채우는 패딩 적용
        # constant_values=1 로 설정하면 경계 밖이 1로 채워집니다.
        padded_map = np.pad(self._visited, pad_width=pad_width, mode='constant', constant_values=1)
        
        # 2. 패딩된 배열에 맞춰 중앙 좌표 수정
        # 사방에 2칸씩 추가되었으므로 모든 좌표에 +2를 해줍니다.
        shifted_centers = np.array([self.agent_pos[i] for i in range(self.n_agents)]) + pad_width
        
        # 3. 상대 오프셋 생성 [-2, -1, 0, 1, 2]
        offsets = np.arange(-pad_width, pad_width + 1)
        
        # 4. 고급 인덱싱으로 배치 추출
        rows = shifted_centers[:, 0, None] + offsets  # (N, 5)
        cols = shifted_centers[:, 1, None] + offsets  # (N, 5)
        ...
        # 5. 패딩된 맵에서 한 번에 추출
        extracted_maps = padded_map[rows_grid, cols_grid]
