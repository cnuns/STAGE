...
def standard_eval(env, policy, dist_adjs, channels, n_episodes=20, greedy=True, load_from_file=False, 
                  render=True, recorder=None, max_steps=200):
        ...
        while not terminated:
            ...
            if not env.centralized:
                ...
                if len(actions.shape) == 3: # n-d action
                    ...
                    actions = actions[:, 0]
                obs, (reward,detail), terminated, info = env.step(actions) # n_env = 1
                ...
                actions, agent_infos = policy.get_actions(obs, 
                    avail_actions, dist_adjs, channels, greedy=greedy)

                obs, (reward,detail), terminated, info = env.step(actions[0]) # n_env = 1
            t += 1
            if t > max_steps:
                terminated = True
