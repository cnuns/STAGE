...
def run(args):
    ...
    if args.mode == 'train':
        ...
        def train_coverage(ctxt=None, args_dict=vars(args)):
            ...
            policy, baseline = gnnOL(args, env)
...
if __name__ == '__main__':
    ...
    args.trEnvStep = 3.5
    args.trMap = args.teMap = args.map
    # args.debug = 1 # this will start training just 3-epochs and testing for each epochs
    #! Run by
    # python runner_co_gnnOL.py --cmd train --map 10 --sen 2 --den 0.04 --loss 0 --fault 0

    if 'train' in args.cmd:
        args, is_trained = ready_to_train(args, scenLib, flag, debug=0) # pass here if model already exist OR start to train new model
        if not is_trained:
            run(args)
