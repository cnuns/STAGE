...
def run(args):
    ...
    if args.mode == 'train':
        ...
        def train_coverage(ctxt=None, args_dict=vars(args)):
            ...
            policy, baseline = stage(args, env)
...
if __name__ == '__main__':
    ...
    if 'train' in args.cmd:
        for map_size, hop_num, step in zip(args.preTrMap, args.preTrHop, args.preTrEnvStep):
            ...
            args.preHop = hop_num
            args.trEnvStep = step
            args, is_trained = ready_to_train(args, scenLib, flag, debug=0)
            if not is_trained:
                run(args)
