...
if __name__ == '__main__':
    ...
    #! Run by
    # python runner_pp_gnnOL.py --cmd train --map 10 --sen 2 --den 0.03 --loss 0 --fault 0

    if 'train' in args.cmd:
        args, is_trained = ready_to_train(args, pp, flag, debug=0) # pass here if model already exist OR start to train new model
        if not is_trained:
            run(args)
