...
class Snapshotter:
    ...
    def __init__(self,
        ...
        self.dir_algo = f'{self._snapshot_dir}/itrs'
        if not os.path.isdir(self.dir_algo):
            os.mkdir(self.dir_algo)
    ...
    def save_itr_params(self, itr, params):
        ...
        if self._snapshot_mode == 'all':
            ...
            if itr % self._snapshot_gap == 0:
                file_name_gap = f'{self.dir_algo}/itr_{str(itr).zfill(4)}.pkl'
                with open(file_name_gap, 'wb') as file:
                    pickle.dump(params['algo'].policy, file)
            ...
            if itr % 20 == 0:
                algo_backup_latest = f'{self._snapshot_dir}/algo_backup.pkl'
                with open(algo_backup_latest, 'wb') as file:
                    pickle.dump(params['algo'], file)
                with open(f'{self._snapshot_dir}/algo_backup_epoch.txt', 'w') as f:
                    f.write(f'itr={itr}')
