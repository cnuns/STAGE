...
class Adam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, amsgrad=False, device='cpu'):
        if not 0.0 <= lr:
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                if p.grad is not None:
                    ...
                    exp_avgs.append(state['exp_avg'].to(self.device))
                    exp_avg_sqs.append(state['exp_avg_sq'].to(self.device))

                    if group['amsgrad']:
                        max_exp_avg_sqs.append(state['max_exp_avg_sq'].to(self.device))
