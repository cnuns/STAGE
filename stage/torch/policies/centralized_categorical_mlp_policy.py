...
class CentralizedCategoricalMLPPolicy(MLPModule):
    ...
    def forward(self, obs_n, avail_actions_n, get_actions=False):
        """
        ...
        if get_actions:
            ...
            if type(obs_n) != torch.Tensor:
                if obs_n.device != torch.device(self.device):
                    obs_n = torch.Tensor(obs_n).to(self.device)
                    avail_actions_n = torch.Tensor(avail_actions_n).to(self.device)
        ...
        if get_actions:
            masked_probs = dists_n.probs.cpu() * avail_actions_n.cpu() # mask
        else:
    ...
    def get_actions(self, obs_n, avail_actions_n, greedy=False):
        ...
        """
        with torch.no_grad():
            dists_n = self.forward(obs_n, avail_actions_n, get_actions=True)
